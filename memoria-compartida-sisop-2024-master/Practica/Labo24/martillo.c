...
void martillo(int id){
    ...
    while(1){
        ...
        printf(AMARILLO"Entro un atleta de martillo con id %d\n",id);
        fflush(stdout);

        usleep(rand()%10000);

        printf(AMARILLO"Se esta yendo el atleta de martillo con id %d \n",id);
        fflush(stdout);
...
int main(int argc,char* argv[]){
    srand(getpid());
    if(argc != 2){
        printf("Error en martillo, 1 atributo.\n");
        return 1;
    }
    int id = atoi(argv[1]);
    martillo(id);
